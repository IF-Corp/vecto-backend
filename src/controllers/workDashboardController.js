...
const getTodaySummary = async (userId) => {
    ...
    const projectsWorked = [...new Set(todayTimesheets.map((t) => t.project_id).filter(Boolean))]
        .length;
...
const getWeekSummary = async (userId) => {
    ...
    const billableHours = billableTimesheets.reduce(
...
const getQuickStats = async (userId) => {
    ...
    const thisMonthHours = thisMonthTimesheets.reduce(
        (sum, t) => sum + (parseFloat(t.hours) || 0),
        0,
    );
    ...
    const lastMonthHours = lastMonthTimesheets.reduce(
        (sum, t) => sum + (parseFloat(t.hours) || 0),
        0,
    );
    ...
    const hoursChangePercent =
        lastMonthHours > 0 ? Math.round((hoursDiff / lastMonthHours) * 100) : 0;
...
const getProductivityScore = async (userId) => {
    ...
    const daysWorked = [...new Set(timesheets.map((t) => format(new Date(t.date), 'yyyy-MM-dd')))]
        .length;
