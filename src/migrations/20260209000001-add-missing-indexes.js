...
module.exports = {
    async up(queryInterface) {
        ...
        await addIndexSafe('transactions', ['category_id'], {
        ...
        await addIndexSafe('routine_execution_items', ['item_id'], {
            name: 'idx_routine_execution_items_item_id',
        });

        // PERFORMANCE: Composite indexes for common queries
        await addIndexSafe('transactions', ['account_id', 'transaction_date'], {
            name: 'idx_transactions_account_date',
        });
        await addIndexSafe('transactions', ['category_id', 'transaction_date'], {
            name: 'idx_transactions_category_date',
        });
        await addIndexSafe('habit_logs', ['habit_id', 'execution_date'], {
            name: 'idx_habit_logs_habit_date',
        });
        await addIndexSafe('work_tasks', ['project_id', 'status_id'], {
            name: 'idx_work_tasks_project_status',
        });
        await addIndexSafe('work_time_entries', ['user_id', 'project_id', 'started_at'], {
            name: 'idx_work_time_entries_user_project_date',
        });
    },
